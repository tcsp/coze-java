...
class MessageServiceTest {
  ...
  void testCreate() throws Exception {
    ...
    when(messageAPI.create(
            eq(conversationId), any(CreateMessageReq.class), any(CreateMessageReq.class)))
        .thenReturn(call);
  ...
  void testList() throws Exception {
    // 准备测试数据
    String conversationId = "test_conversation_id";
    ListMessageReq req = ListMessageReq.builder().conversationID(conversationId).limit(10).build();
    ...
    ListMessageResp listResp =
        ListMessageResp.builder().data(messages).firstID("msg1").lastID("msg2").build();

    BaseResponse<ListMessageResp> baseResponse =
        BaseResponse.<ListMessageResp>builder()
            .code(0)
            .msg("success")
            .logID(Utils.TEST_LOG_ID)
            .data(listResp)
    ...
    when(messageAPI.list(eq(conversationId), any(ListMessageReq.class), any(ListMessageReq.class)))
        .thenReturn(call);
    when(call.execute())
        .thenReturn(Response.success(baseResponse.getData(), Utils.getCommonHeader()));
  ...
  void testRetrieve() throws Exception {
    ...
    RetrieveMessageReq req =
        RetrieveMessageReq.builder().conversationID(conversationId).messageID(messageId).build();

    Message message = Message.builder().id(messageId).content("Test message").build();
  ...
  void testDelete() throws Exception {
    ...
    DeleteMessageReq req =
        DeleteMessageReq.builder().conversationID(conversationId).messageID(messageId).build();
