...
class WorkflowRunHistoryServiceTest {
    ...
    @Mock
    private WorkflowRunHistoryAPI historyAPI;
    ...
    void testRetrieve() throws Exception {
        ...
        RetrieveRunHistoryReq req = RetrieveRunHistoryReq.builder()
                .workflowID(workflowId)
                .executeID(executeId)
                .build();

        List<WorkflowRunHistory> histories = Arrays.asList(
                WorkflowRunHistory.builder().executeID("node1").build(),
                WorkflowRunHistory.builder().executeID("node2").build()
        );

        BaseResponse<List<WorkflowRunHistory>> baseResponse = BaseResponse.<List<WorkflowRunHistory>>builder()
                .code(0)
                .msg("success")
